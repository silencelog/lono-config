...
class Config {
  ...
  install (lono) {
    if (lono.context.hasOwnProperty(CONTEXT_CONFIG)) return
    // 核心注入
    this.opts.forEach((opt) => {
      ...
      // 读取配置中心配置
        ...
        // if (lono.$observer) {
        //   lono.$observer.on(`$eureka:mounted`, () => {})
        // }
        // config = opt.config
      // 读取数据库配置
      } else {
    ...
    Object.defineProperty(lono, '$config', {
      value: this.config,
      writable: false
    })
    Object.defineProperties(lono.context, {
      [CONTEXT_CONFIG]: {
        value: lono.$config,
        writable: false
      },
  ...
  setConfig (lono, config) {
    lono.$observer.emit(`$config:update`, config)
  }
  async loadJSON (url) {
